...
class PseudoLabeler(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, model, valid,test, features, target, sample_rate=0.8, seed=20):
    ...
    def fit(self, X, y):
        if self.sample_rate > 0.0:
            ...
            t=10
            loss=0
            numone=int(self.sample_rate*20000)
            ...
            for i, (train_index, test_index) in enumerate(skf.split(tr_x, tr_y)):
                ...
                lable=np.vstack((y[train_index],ltest)).ravel()
                val_x=x[test_index]#验证集数据全都来自test_all
                val_y=y[test_index].ravel()
                aa.append(i)
            
                print("Fold", i)
                self. model.fit(train,lable,
...
#        test_pred=test_pred/5
        ...
        return test_pred,aa,baseloss


    def __create_augmented_train(self, X, y):
        numone=450#pseudo-lable中的最大的numone个数据为1，与train_xy同分布
        num_of_samples = int(len(test) * self.sample_rate*2)
...
#    #predict_result['pred_prob']=predict_result['pred_prob']+train_p
            if loss<t:
                t=loss
                train_p=self.model.predict(train, num_iteration=self.model.best_iteration_)
                test_pred= self.model.predict(test, num_iteration=self.model.best_iteration_)
                valid_pred= self.model.predict(valid, num_iteration=self.model.best_iteration_)
        #predict_result['pred_prob']  = train_p
...
#        valid_pred=np.abs(valid_pred/5)
#        train_p=np.abs(train_p/5)
#        test_pred=np.abs(test_pred/5)
        pseudo_labels = valid_pred
        #np.vstack((test_pred,valid_pred)).reshape((20000,1))
        #print(pseudo_lables)
...
#        augmented_test =test.copy(deep=True)
        ...
        return sampled_test ,temp_train
...
re=pd.DataFrame()
re['cust_id']=test['cust_id']
target = 'y'
test.drop(['y','cust_id'],axis=1,inplace=True)
valid.drop(['y','cust_id'],axis=1,inplace=True)
...
model = PseudoLabeler(
     lgb.LGBMRegressor(boosting_type='gbdt', num_leaves=20, max_depth=-1, learning_rate=0.01, n_estimators=5000,
...
result,aa,cc=model.fit(X_train, y_train)
print(cc)
#result即为test_all的预测标签
#a=model.predict(X_test)
re['pred_prob']=result
re.to_csv(r'C:\Users\Administrator\Desktop\sub.csv',index=False)



#class PseudoLabeler(BaseEstimator, RegressorMixin):
#    
#    def __init__(self, model, valid,test, features, target, sample_rate=0.5, seed=42):
#        self.sample_rate = sample_rate#取值范围为0.0-1.0
#        self.seed = seed
#        self.model = model
#        self.model.seed = seed
...
#                train=np.vstack((x[train_index],ldata))
#                print(y[train_index].shape,ltest.shape)
#                lable=np.vstack((y[train_index],ltest)).tolist()
#                val_x=x[test_index]#验证集数据全都来自test_all
#                val_y=y[test_index].tolist()
#                aa.append(i)
#          
#            print("Fold", i)
...
#                          eval_metric='auc',
#                          eval_set=[(train, lable), 
#                                    (val_x, val_y)],early_stopping_rounds=60)
#            baseloss.append(self.model.best_score_['valid']['auc'])
#            loss += self.model.best_score_['valid']['auc']
##            train_p=self.model.predict_proba(train, num_iteration=self.model.best_iteration_)[:, 1]
##            test_pred= self.model.predict_proba(test, num_iteration=self.model.best_iteration_)[:, 1]
...
#                t=loss
#                train_p=self.model.predict(x, num_iteration=self.model.best_iteration_)
#                test_pred= self.model.predict(self.test, num_iteration=self.model.best_iteration_)
#        #predict_result['pred_prob']  = train_p
##            
##            print('test mean:', test_pred.mean())
##            sampled_test = augmented_test.sample(n=num_of_samples)
...
#        return test_pred
#    
#    def __create_augmentedray_train(self, X, y):
#        numone=922#pseudo-lable中的最大的numone个数据为1，与train_xy同分布
#        num_of_samples = int(len(test) * self.sample_rate)+int(len(valid) * self.sample_rate)
#        #从valid和test数据中分别选出sample_rate比例的数据加入训练
#        skf = StratifiedKFold(n_splits=5, random_state=20, shuffle=True)
#        baseloss = []
#        loss = 0
#        t=10
...
#                          eval_metric='auc',
#                          eval_set=[(train[train_index], lable[train_index]), 
#                                    (train[test_index], lable[test_index])],early_stopping_rounds=2)
#            baseloss.append(self.model.best_score_['valid']['auc'])
#            loss += self.model.best_score_['valid']['auc']
#            train_p=self.model.predict(train, num_iteration=self.model.best_iteration_)
#            test_pred= self.model.predict(test, num_iteration=self.model.best_iteration_)
